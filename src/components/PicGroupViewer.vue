<template>
	 <div class="pic-group-info-list-wrapper">
        <div class="pic-group-info-wrapper" @click="showDetail(picGroup.id, pic.groupIds, index)" v-for="(picGroup, index) in picGroups" v-bind:key="picGroup.id" v-show="picShowType==='WIDTH_EQUAL'">
		...
		<div id="picGroupDiv" class="well-chosen-list-box-height" v-show="picShowType==='HEIGHT_EQUAL'">
			  <div @click="showDetail(pic.id, pic.groupIds, index)" class="well-chosen-info-box-height jss117" v-for="(pic, index) in list" v-bind:key="pic.id">
...
export default {
  ...
  methods: {
	  ...
	  showDetail(id, groupIds, index) {
		  if(this.searchType !== 'SINGLE') {
			  this.$router.push('/group-pictures?id=' + id)
		  } else {
			  if(this.$route.path === '/picture-detail') {
				  this.$emit('updatePicIndex', index)
				  document.getElementById('imageLayer').scrollIntoView(true);
			  } else {
				  var groupId = groupIds.split(',')[0]
				  this.$router.push('/picture-detail?groupId=' + groupId + '&picId=' + id)
			  }
