  ...
  <div class="login_div">
    ...
    >
      ...
      <a-form-item label="password">
        <a-popover
          title="密码至少包含:"
          :visible="passCheckVisible"
          @click="passCheckVisible = true"
          trigger="click"
          ...
          <template slot="content">
            <div>
              <ul class="password_checks">
                <li
                  v-for="(passCheckItem, i) in passCheckItems"
                  v-bind:key="passCheckItem.name"
                >
                  <svg
                    :class="{
                      password_check_correct: passCheckItemStatuses[i],
                      password_check_error: !passCheckItemStatuses[i],
                      password_check_status: true,
                      'MuiSvgIcon-root': true,
                      'MuiSvgIcon-fontSizeLarge': true,
                    }"
                    focusable="false"
                    viewBox="0 0 24 24"
                    aria-hidden="true"
      ...
      <a-form-item>
        <a-button
          size="large"
          style="height: 56px; background-color: #5986ff; border-color: #5986ff"
          font-size="25"
          class="input_field"
          type="primary"
          html-type="submit"
    ...
    <div class="create_account_div">
      <div>
        <router-link
          class="MuiTypography-root MuiLink-root MuiLink-underlineHover aclass"
          to="/login"
          >已经有一个账户？登录</router-link
        >
      </div>
    </div>
  </div>
...
export default {
      ...
      passCheckItems: [
        {
          tip: "8~14个字符",
          name: "word_limit",
          checkFunc: (a) => a.length >= 8 && a.length <= 14,
          status: false,
        },
        {
          tip: "数字",
          name: "digit",
          checkFunc: (a) => a.search(/\d+/) >= 0,
          status: false,
        },
        {
          tip: "大写字母",
          name: "upper_case",
          checkFunc: (a) => a.search(/[A-Z]+/) >= 0,
          status: false,
        },
        {
          tip: "小写字母",
  ...
  methods: {
    ...
    register(e) {
      e.preventDefault();
      this.passCheckVisible = !this.passCheckItemStatuses.reduce(
        (a, b) => a && b
      );
