  ...
  <div class="MuiToolbar-root MuiToolbar-regular page_main MuiToolbar-gutters">
      ...
      <div class="content">
        <div class="upload-container"> 
          <div
            ...
            :class="{
              'image-preview': resourceType !== 'vector',
              selected: image.uid === selectedImageUid,
              vector: resourceType === 'vector',
            }"
            @click="selectImage(image.uid, index)"
          >
            ...
            <div class="image-preview-wrapper">
              ...
              <a-popover
                v-if="resourceType === 'vector' && image.url"
                placement="rightBottom"
              >
                <template slot="content">上传源文件</template>
                <a-button
                  class="pre_file_upload_btn"
                  alt="上传源文件"
                  @click="triggerSourceUpload(image.uid)"
                >
                  <a-icon
                    type="upload"
                    :style="{ color: '#ffffff' }"
                ...
                <input
                  ...
                  type="file"
                  :ref="'sourceFile' + image.uid"
                  @change="setSourceFile($event, index)"
                />
              </a-popover>
              <img
                class="vector"
                v-if="resourceType === 'vector'"
                :src="image.url"
              />
              ...
              <a-progress
                type="circle"
                v-if="resourceType !== 'vector' && uploading && !image.url"
          ...
          >
            <div class="choose_picture">
              <a-icon
                :style="{ marginTop: '56px', height: '0px' }"
                size="large"
                type="plus"
              ></a-icon>
              <span> 请选择{{ resourceTypeName }} </span>
            </div>
          </el-upload>
        </div>
      </div>
    ...
    <div class="right_wrapper">
      <div class="header">
        ...
        <a-button :disabled="!canCommit" type="danger" @click="save(false)"
          >提交</a-button
        >
      </div>
      <div class="content">
        <a-form-model
          ...
          :rules="uploadInfoFormRules"
          style="text-align: left"
        >
          <div class="group_title">组信息</div>
          <a-form-model-item label="组照标题" prop="groupTitle">
            <a-input
              size="large"
              v-model="uploadInfoForm.groupTitle"
              placeholder="请输入组照标题"
            ></a-input>
          </a-form-model-item>
          <a-form-model-item label="组照说明" prop="groupCaption">
            <a-input
              size="large"
              v-model="uploadInfoForm.groupCaption"
              placeholder="请输入组照说明"
            ></a-input>
          </a-form-model-item>
          ...
          <a-form-model-item label="组照关键词" prop="groupKeywords">
            <div
              class="
                ...
                jss207
                jss210
                MuiInputBase-formControl MuiInputBase-adornedStart
            ...
            >
              ...
              >
                ...
                <svg
                  ...
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                  @click="removeKeyword('groupKeywordArr', index)"
                >
                  <path
              ...
              <a-input
                size="large"
                @blur="createKeywordLabel('groupKeywords')"
                @pressEnter="createKeywordLabel('groupKeywords')"
                @change="changeKeywords($event, 'groupKeywords')"
                v-model="uploadInfoForm.groupKeywords"
                placeholder="请输入关键词,并空格或回车"
              ></a-input>
            </div>
          </a-form-model-item>

          <div class="group_title">{{ resourceTypeName }}信息</div>
          <a-form-model-item label="摄影师">
            <a-input
              size="large"
              v-model="uploadInfoForm.assetsList[selectedImageIndex].creditLine"
              placeholder="请输入摄影师"
            ></a-input>
          </a-form-model-item>
          <a-form-model-item label="署名">
            <a-input
              size="large"
              v-model="uploadInfoForm.assetsList[selectedImageIndex].signature"
              placeholder="请输入署名"
            ></a-input>
          </a-form-model-item>
          <a-form-model-item label="拍摄地点">
            <a-select
              default-value="0"
              v-model="
                uploadInfoForm.assetsList[selectedImageIndex].firstCityIndex
              "
            ...
            <a-select
              default-value="0"
              v-model="
                uploadInfoForm.assetsList[selectedImageIndex].secondCityIndex
              "
              style="width: 120px"
            >
              <a-select-option
                v-for="(city, index) in provinceData[
                  uploadInfoForm.assetsList[selectedImageIndex].firstCityIndex
                ].children"
                :key="index"
              >
            ...
            <a-input
              size="large"
              v-model="uploadInfoForm.assetsList[selectedImageIndex].location"
              placeholder="请输入拍摄地点"
            ></a-input>
          </a-form-model-item>
          <a-form-model-item label="拍摄时间">
            <a-date-picker
              size="large"
              v-model="uploadInfoForm.assetsList[selectedImageIndex].shootTime"
              placeholder="请输入拍摄时间"
              valueFormat="x"
            />
          </a-form-model-item>
          <a-form-model-item :label="resourceTypeName + '说明'">
            <a-textarea
              :rows="2"
              size="large"
              v-model="uploadInfoForm.assetsList[selectedImageIndex].caption"
              placeholder="点击输入说明(1000字以内)说明需要包括年、月、日、地点，画面本身的内容"
            ></a-textarea>
          </a-form-model-item>
          <a-form-model-item label="关键词">
            ...
            >
              <div
                v-for="(keyword, index) in uploadInfoForm.assetsList[
                  selectedImageIndex
                ].keywordArr"
                v-bind:key="keyword"
                class="
              ...
              >
                ...
                <svg
                  ...
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                  @click="removeAssetKeyword('keywords', 'keywordArr', index)"
                >
                  <path
                    d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"
                  ></path>
              ...
              <a-input
                class="ant-input-keyword"
                size="large"
                @blur="createAssetKeywordLabel('keywords', 'keywordArr')"
                @pressEnter="createAssetKeywordLabel('keywords', 'keywordArr')"
                @change="changeAssetKeywords($event, 'keywords', 'keywordArr')"
                v-model="uploadInfoForm.assetsList[selectedImageIndex].keywords"
                placeholder="请输入关键词,并空格或回车"
              ></a-input>
            </div>
          </a-form-model-item>
            ...
            >
              ...
              >
                ...
                <svg
                  ...
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                  @click="removeAssetKeyword('people', 'peopleArr', index)"
                >
                  <path
                    d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"
                  ></path>
              ...
              <a-input
                class="ant-input-personal"
                size="large"
                @blur="createAssetKeywordLabel('people', 'peopleArr')"
                @pressEnter="createAssetKeywordLabel('people', 'peopleArr')"
                @change="changeAssetKeywords($event, 'people', 'peopleArr')"
                v-model="uploadInfoForm.assetsList[selectedImageIndex].people"
                placeholder="请输入关键词,并空格或回车"
              ></a-input>
            </div>
          </a-form-model-item>
    ...
    <a-modal
      title="确定删除"
      :visible="deleteImgVisible"
      @ok="deleteImg"
      @cancel="deleteImgVisible = false"
    >
      您确定要删除该资源？
    </a-modal>
  </div>
...
export default {
  data() {
    return {
      ...
      uploadInfoFormRules: {
        groupTitle: [{ required: true, message: "组照标题不能为空" }],
        groupCaption: [{ required: true, message: "组照说明不能为空" }],
        categoryId: [{ required: true, message: "请选择一个组照分类" }],
        groupKeywords: [{ required: true, message: "请填写组照关键词" }],
      },
      loading: false,
      listObj: {},
      fileList: [],
  ...
  methods: {
    ...
    save(isDraft) {
      ...
      if (
        this.uploadInfoForm.assetsList.length === 1 &&
        !this.uploadInfoForm.assetsList[0].ossYuantu
      ) {
      ...
      this.uploadInfoForm.groupKeywords =
        this.uploadInfoForm.groupKeywordArr.join(",");
        ...
        if (valid) {
          for (var i in this.uploadInfoForm.assetsList) {
            if (this.uploadInfoForm.assetsList[i].isSurface) {
              this.uploadInfoForm.surfacePhoto =
                this.uploadInfoForm.assetsList[i].ossYuantu;
              break;
            }
          }
          ...
          uploadInfoForm.assetsList.forEach((item, index) => {
            ...
            var city =
              this.provinceData[item.firstCityIndex].children[
                item.secondCityIndex
              ].label;
            if (province !== "请选择") {
              item.area += "/" + province;
            }
    ...
    createKeywordLabel(keywordField) {
      ...
      this.uploadInfoForm[
        keywordField.substring(0, keywordField.length - 1) + "Arr"
      ].push(this.uploadInfoForm.groupKeywords.trim());
  ...
  created() {
    ...
    this.uploadInfoForm.assetType =
      resourceTypeInfos[this.resourceType].assetType;
